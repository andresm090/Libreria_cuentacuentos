...
var sendRequest = function(path, params, callback) {
    ...
    https.get(url, function(response) {
        ...
        response.on('end', function() {
            ...
            /*} catch (e) {
                callback(new Error('ERROR!!! Invalid response from Google Books API.'));
            }*/
...
var parseBook = function(data) {
    ...
    /*var book = _.pick(data.volumeInfo, [
        ...
        'ratingsCount', 'maturityRating', 'saleInfo', 'language'
    ]);*/

    // ISBN
    /*var isbn, identifiers = data.volumeInfo.industryIdentifiers
    ...
    if (data.volumeInfo.industryIdentifiers && data.volumeInfo.industryIdentifiers.length > 0) {
      var isbn13 = _.find(data.volumeInfo.industryIdentifiers, function (item) {
    ...
    //PRECIO
    if (data.volumeInfo.saleInfo && data.volumeInfo.saleInfo.retailPrice) {
      book.price = data.volumeInfo.saleInfo.retailPrice.amount;
    }
    else {
      if (data.volumeInfo.saleInfo && data.volumeInfo.saleInfo.listPrice) {
        book.price = data.volumeInfo.saleInfo.listPrice.amount;
      }
    }
//price: data.saleInfo.retailPrice.amount, price: _.get(data, 'volumeInfo.saleInfo.retailPrice.amount')
    _.extend(book, {
        id: data.id,
